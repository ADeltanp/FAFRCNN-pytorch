...
def create_grid(img_size, base_size=16, scales=t.tensor([16, 10, 6]), ratios=t.tensor([0.5, 1, 2])):
    """
    :param img_size: (int tuple) h * w of the image
    ...
    :return: (list of t.Tensor) grids of different sizes and ratios, for the same size and ratio, its shape is (G, 4), G
    is the number of grids generated of current size(scale). Each is (x_l_top, y_l_top, x_r_bottom, y_r_bottom) on the
    image.
    """
    grids = list()
    for i in range(len(scales)):
        ...
        for j in range(len(ratios)):
            w = int(base_size * scales[i] * (ratios[j] ** 0.5))
            h = int(base_size * scales[i] / (ratios[j] ** 0.5))

            sx = random.randint(0, w)
            sy = random.randint(0, h)
            base_coord = t.tensor([sx, sy, sx + w, sy + h]).int()

            h_slices = (img_size[0] - sy) // h
            w_slices = (img_size[1] - sx) // w

            x_coord = t.tensor([w * k for k in range(w_slices)]).int()
            y_coord = t.tensor([h * k for k in range(h_slices)]).int()
