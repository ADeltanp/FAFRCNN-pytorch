    ...
    for epoch in range(opt.transfer_epoch):
        ...
        for ii, (s_, t_) in tqdm(enumerate(zip(source_dataloader, cycle(target_dataloader)))):
            ...
            if (ii + 1) % opt.plot_every == 0:
                ...
                # plot gt target bboxes
                t_ori_img_ = inverse_normalize(at.tonumpy(t_img[0]))
                gt_img = visdom_bbox(t_ori_img_,
                                     at.tonumpy(t_bbox_[0]),
                                     at.tonumpy(t_label_[0]))
                trainer.vis.img('target_img', gt_img)

                # plot predicted target bboxes
                _bboxes, _labels, _scores = trainer.faster_rcnn.predict([t_ori_img_], visualize=True)
                pred_img = visdom_bbox(t_ori_img_,
                                       at.tonumpy(_bboxes[0]),
                                       at.tonumpy(_labels[0]).reshape(-1),
                                       at.tonumpy(_scores[0]))
                trainer.vis.img('target_pred', pred_img)
